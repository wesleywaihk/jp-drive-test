...
const LEVEL_LABEL: Record<1 | 2, string> = {
  1: "Bookmarked",
  2: "Very Important",
};
...
export default function BookmarkScreen({
  ...
  const bookmarked = questions
    ...
    .filter((e) => e.level >= 1)
    .sort((a, b) =>
      a.question.id.localeCompare(b.question.id, undefined, { numeric: true }),
    );
  ...
  const pageItems = bookmarked.slice(start, start + PAGE_SIZE);

  const confirmQuestion = confirmId
    ? questions.find((q) => q.id === confirmId)
    : null;
